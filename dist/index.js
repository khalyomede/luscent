...
var bindEvents = function(context, getters, methods, conditions, lists, element) {
	var target = element !== null && element !== void 0 ? element : document;
	target.querySelectorAll("[data-luscent-on-click]").forEach(function(element$1) {
		var methodName = element$1.getAttribute("data-luscent-on-click");
		if (methodName && methods[methodName]) element$1.addEventListener("click", function(event) {
			var id = element$1.dataset.luscentRenderedId;
			context.state = methods[methodName](context.state, event, id);
			update_dom_default(context, getters, methods, conditions, lists);
		});
	});
	target.querySelectorAll("[data-luscent-on-submit]").forEach(function(element$1) {
		var methodName = element$1.getAttribute("data-luscent-on-submit");
		if (methodName && methods[methodName]) element$1.addEventListener("submit", function(event) {
